...
class WorkspaceResearchCapability(BaseTool):
    ...
    @tool(
        ...
        description="""
            ...
            注意事项：
            - 使用“单目标、单主题”的自然语言表达（问句或陈述句均可），首轮只保留主题本身，不附加次要条件
            - 不要把输入压缩成碎片化词串；避免只写名词堆叠、标签堆叠或多个并列意图混在一条查询里
            - 禁止关键词式输入示例：
              `上海 旅游 攻略 2天 预算3000`、
              ...
              `上海 上牌 流程 新能源 2026`、
              `项目管理 工具 看板 协作 推荐`
            - 主题优先原则：第一轮先查主题，不加时间、地点、预算、版本、人群等附加约束，先拿到稳定候选集合
            - 渐进收敛原则：仅在结果不够准确时，再逐轮增加一个筛选条件，避免一次加入多个条件导致召回下降
            - 若用户任务包含多个子问题，必须拆成多次搜索；每次只提交一个可验证的问题，按结果逐步推进
        ...
        parameters={
            "query": {
                "type": "string",
                "description": "一条原子性的自然语言检索请求，只描述当前要验证的单一主题。首轮请只写主题，不附加次要约束；若结果不够再逐轮补充一个条件。"
                               "表达形式可用问句或陈述句。"
                               "正确示例：'AI编程助手'、'主流项目管理工具'、'新能源汽车上牌流程'。"
