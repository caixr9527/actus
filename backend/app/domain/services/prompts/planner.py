...
- 步骤以执行动作为主，不要把最后一步规划成长篇总结、完整成稿或大段最终正文
- 详细、完整的最终整理主要由 `summary` 阶段完成，不要单独规划一个只负责长篇成稿的最后步骤
- 允许“执行过程型整理”步骤，例如整理中间结构、归档事实、生成导出文件
- 允许步骤给出简短直接的结果表达，但不要把它写成长篇最终回答
- 禁止示例：
  - `整理 5 条要点，写成完整攻略后输出给用户`
  - `汇总上述结果形成最终完整回答`
  - `根据已有信息撰写最终说明成稿`
- 正确示例：
  - `检索相关来源并记录候选链接`
  - `读取关键页面并提取实现细节与来源`
  - `整理已收集的事实并生成 Markdown 文件`
  - `将结果导出为 Markdown 文件`
  - `基于已完成结果生成简短步骤结论，并产出交付文件`
...
- 允许“执行过程型整理”步骤，例如整理中间结构、归档事实、生成导出文件
- 禁止“最终用户交付型整理”步骤，也就是只基于已有结果形成最终回答、最终要点或面向用户的最终说明
- 禁止示例：
  - `整理 5 条要点并输出给用户`
  - `汇总上述结果形成最终回答`
  - `根据已有信息撰写最终说明`
