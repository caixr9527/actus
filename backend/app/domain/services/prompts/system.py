...
- 调用 `search_web` 时必须使用单主题自然语言查询，像向搜索引擎提出一个清晰问题或短句；不要把多个短语、属性或筛选条件拆成关键词堆叠
- `search_web` 返回的摘要（Snippets）可以作为初步判断和候选来源线索；当摘要不足以支撑结论、需要核验细节或用户要求来源时，再读取原始页面
- 多轮检索时每轮只增加一个明确筛选条件，不要把实体、属性、价格、时间、地区等切成多组短语分别搜索
</search_rules>
