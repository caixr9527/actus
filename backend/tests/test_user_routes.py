...
class _ErrorUserService:
    async def get_current_user_profile(self, user_id: str):
        raise BadRequestError(
            "用户不存在，请重新登录",
            error_key=error_keys.USER_NOT_FOUND,
        )

    async def update_current_user_profile(self, user_id: str, updates: dict[str, str | None]):
        raise BadRequestError(
            "至少需要更新一个字段",
            error_key=error_keys.USER_PROFILE_UPDATE_EMPTY,
        )
    ...
    async def update_current_user_password(
        ...
        raise BadRequestError(
            "旧密码错误",
            error_key=error_keys.USER_CURRENT_PASSWORD_INCORRECT,
        )
...
def test_get_current_user_profile_route_should_map_bad_request_error() -> None:
    app = FastAPI()
    register_exception_handlers(app)
    app.include_router(users_router, prefix="/api")
    ...
    assert payload["msg"] == "用户不存在，请重新登录"
    assert payload["error_key"] == error_keys.USER_NOT_FOUND


def test_update_current_user_profile_route_should_map_bad_request_error() -> None:
    app = FastAPI()
    register_exception_handlers(app)
    app.include_router(users_router, prefix="/api")
    app.dependency_overrides[get_user_service] = lambda: _ErrorUserService()
    ...
    payload = response.json()
    assert payload["code"] == 400
    assert payload["msg"] == "至少需要更新一个字段"
    assert payload["error_key"] == error_keys.USER_PROFILE_UPDATE_EMPTY
...
def test_update_current_user_password_route_should_map_bad_request_error() -> None:
    ...
    assert payload["error_key"] == error_keys.USER_CURRENT_PASSWORD_INCORRECT
